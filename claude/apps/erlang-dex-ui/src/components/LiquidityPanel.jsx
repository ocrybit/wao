    ...
    <div className="liquidity-panel" data-testid="liquidity-panel">
      ...
      <div className="positions-list" data-testid="positions-list">
        ...
        {positions.length === 0 ? (
          ...
          <ul>
            {positions.map((pos) => (
              <li key={pos.pool_id} data-testid={`position-${pos.pool_id}`}>
                <div className="position-info">
                  <strong>{pos.pool_id}</strong>
                  <p>Liquidity: {pos.liquidity}</p>
                  <p>Share: {pos.share?.toFixed(2)}%</p>
                  <p>Value A: {pos.value_a}</p>
                  <p>Value B: {pos.value_b}</p>
                </div>
                <div className="position-actions">
                  <input
                    ...
                    min="1"
                    max="100"
                    data-testid={`remove-percent-${pos.pool_id}`}
                  />
                  <span>%</span>
                  <button
                    onClick={() => handleRemoveLiquidity(pos.pool_id)}
                    disabled={loading}
                    data-testid={`remove-liquidity-${pos.pool_id}`}
                  >
                    Remove
                  </button>
                </div>
