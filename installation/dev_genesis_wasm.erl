...
%%% Timeout for legacy CU status check (2 seconds to handle proxy latency).
...
gateway_env_vars() ->
    ...
    % Filter out undefined/false values (os:getenv returns false if not set)
    [{K, V} || {K, V} <- Vars, V =/= false].