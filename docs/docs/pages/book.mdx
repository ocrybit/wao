...
Now that you understand the codebase, build your own devices. This chapter provides progressive tutorials from beginner to master level.

---

### Device Building Tutorials

Complete these tutorials in order. Each builds on concepts from the previous level.

| Level | Tutorial | What You'll Build |
|-------|----------|-------------------|
| L1 | [Key-Value Store](/book/build1) | Simple storage device using `hb_ao`, `hb_private`, `hb_cache`, and `hb_store` |
| L2 | [Data Processor](/book/build2) | Codec device using `hb_message`, `dev_codec_*`, `hb_link`, and `hb_maps` |
| L3 | [API Gateway](/book/build3) | Authenticated gateway with payment, scheduling, and routing |
| L4 | [Data Platform](/book/build4) | Decentralized storage with Arweave bundles and node replication |
| L5 | *JS Smart Contract Platform* | Master-level: JavaScript execution via WASM/QuickJS (coming soon) |
