...
Add tests to your module using `hb_ao:resolve` with device stack composition:

```erlang
-ifdef(TEST).
-include_lib("eunit/include/eunit.hrl").

%% Test device info via hb_ao:resolve with {as, Module, Msg}
info_test() ->
    application:ensure_all_started(hb),
...
%% Test WASM-based token contract using device stack
aojs_token_test_() ->
    {timeout, 60, fun aojs_token_test/0}.

aojs_token_test() ->
    application:ensure_all_started(hb),
    Opts = #{store => hb_opts:get(store)},
    ...
    %% Initialize
    {ok, M1} = hb_ao:resolve(Stack, #{<<"path">> => <<"init">>}, Opts),

    %% Get token info
    InfoMsg = #{<<"path">> => <<"compute">>, <<"body">> => #{<<"action">> => <<"Info">>}},
    {ok, InfoResult} = hb_ao:resolve(M1, InfoMsg, Opts),
    ?assertMatch(#{}, InfoResult),
    ...
    {ok, MintResult} = hb_ao:resolve(InfoResult, MintMsg, Opts),
    ...
    {ok, BalanceResult} = hb_ao:resolve(MintResult, BalanceMsg, Opts),
    ...
    {ok, _TransferResult} = hb_ao:resolve(BalanceResult, TransferMsg, Opts),
    ok.
...
describe("L5 - JavaScript Smart Contract Platform (aojs@1.0)", function () {
  ...
  it("should return device info", async () => {
    ...
    assert.ok(info.description.includes("JavaScript"))
  })
