  ...
  useEffect(() => {
    const getQuote = async () => {
      ...
      try {
        ...
        // Erlang device returns lowercase keys: amount_out, price_impact
        if (result.amount_out !== undefined) {
          setQuote(result)
          setError(null)
        }
      } catch (err) {
    ...
    <div className="swap-panel" data-testid="swap-panel">
      ...
      {quote && (
        <div className="quote-info" data-testid="quote-info">
          <p>Rate: 1 {tokenIn} = {(quote.amount_out / Number(amountIn)).toFixed(4)} {tokenOut}</p>
          <p>Price Impact: {((quote.price_impact || 0) / 100).toFixed(2)}%</p>
          <p>Fee: {quote.fee || 0} {tokenIn}</p>
        </div>
      )}
