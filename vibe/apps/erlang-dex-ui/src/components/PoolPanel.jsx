    ...
    <div className="pool-panel" data-testid="pool-panel">
          ...
          <ul>
            {pools.map((pool) => (
              <li
                ...
                className={selectedPool?.id === pool.id ? 'selected' : ''}
                data-testid={`pool-${pool.id}`}
              >
                <strong>{pool.id}</strong>
                <span>TVL: {pool.reserve_a + pool.reserve_b}</span>
                <span>Swaps: {pool.swap_count || 0}</span>
              </li>
            ))}
          </ul>
        )}
      ...
      {selectedPool && (
        <div className="pool-details" data-testid="pool-details">
          <h3>Pool Details</h3>
          <p><strong>ID:</strong> {selectedPool.id}</p>
          <p><strong>Token A:</strong> {selectedPool.token_a}</p>
          <p><strong>Token B:</strong> {selectedPool.token_b}</p>
          <p><strong>Reserve A:</strong> {selectedPool.reserve_a}</p>
          <p><strong>Reserve B:</strong> {selectedPool.reserve_b}</p>
          <p><strong>Price:</strong> {selectedPool.price?.toFixed(6)}</p>
          <p><strong>TVL:</strong> {selectedPool.tvl}</p>
          <p><strong>Total Liquidity:</strong> {selectedPool.total_liquidity}</p>
        </div>
      )}
