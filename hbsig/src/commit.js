...
export const commit = async (obj, opts) => {
  ...
  // Use base64url encoding to match the signature-input header keyid
  const base64url = pub.toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "")
  const keyid = `publickey:${base64url}`
