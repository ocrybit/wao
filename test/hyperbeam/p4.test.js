...
describe("Hyperbeam", function () {
  ...
  // SKIP: HyperBEAM's JSON codec applies type conversion (ao-types) BEFORE signature verification
  // See: test/hyperbeam/fail/p4-fail-1.test.js
  it.skip("should handle payment with lua (type conversion issue)", async () => {
    const port = 10002
    const addr2 = toAddr(acc[0].jwk.n)
    const process = hbeam.file("scripts/hyper-token-p4.lua")
