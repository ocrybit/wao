...
describe("Router Device Comprehensive Test Suite", function () {
  ...
  describe("Routes Management", () => {
    ...
    it("should add a new route", async () => {
      const out = await hb.p("/~router@1.0/routes", {
        template: "/new/.*",
        node: "new-server",
        priority: 5,
      })
    ...
    it("should reject unauthorized additions", async () => {
      ...
      try {
        await unauthorizedHB.p("/~router@1.0/routes", {
    ...
    it("should support multiple route owners", async () => {
      ...
      const unauthorizedHB = await new HB({}).init(acc[1].jwk)
      const out = await unauthorizedHB.p("/~router@1.0/routes", {
  ...
  describe("Route Resolution", () => {
    ...
    it("should route by regex priority", async () => {
      const res1 = await hb.g("/~router@1.0/route", {
        "route-path": "/api/v2/users",
      })
      assert.equal(res1, "api-v2")

      const res2 = await hb.g("/~router@1.0/route", {
        "route-path": "/api/v1/users",
      })
      assert.equal(res2, "api-v1")

      const res3 = await hb.g("/~router@1.0/route", { "route-path": "/other" })
      assert.equal(res3, "default")
    })
    ...
    it("should handle explicit HTTP URLs", async () => {
      ...
      console.log("Explicit URL result:", res)
      ...
      assert(res === "https://example.com/api" || res === "default")
    })
    ...
    it("should handle no matches", async () => {
      ...
      try {
        await hb.g("/~router@1.0/route", { "route-path": "/nomatch" })
  ...
  describe("Route Transformations", () => {
    it("should apply prefix transformation", async () => {
      ...
      assert.equal(res, "https://api.backend.com")
    })
    ...
    it("should apply regex replacement", async () => {
      ...
      assert.equal(res, "transformed-server")
    })
  })
  ...
  describe("Preprocessing", () => {
    it("should preprocess requests", async () => {
      await hb.p("/~meta@1.0/info", {
        routes: [{ template: "/remote/.*", node: "remote-server" }],
      })

      const res = await hb.p("/~router@1.0/preprocess", {
        request: {
          path: "/remote/api",
          method: "GET",
        },
  ...
  describe("Complex Scenarios", () => {
    it("should handle multi-strategy routing", async () => {
      ...
      assert.equal(fallbackRes, "fallback")
    })
  })
